...
async function loadData(authUser, silent = false) {
  ...
  try {
    ...
    if (authUser) {
      ...
      if (!currentUser) {
        // En caso de que el trigger haya fallado o no esté configurado, hacemos un fallback
        ...
        let newRol = S.usuarios.length === 0 ? 'anciano' : 'hermano';
        ...
        // Si el usuario existe pero es la primera vez que entra, le preguntamos su nombre
        // si el trigger le puso por defecto la parte del email
        const nombrePorDefecto = authUser.email.split('@')[0];
        if (currentUser.nombre === nombrePorDefecto) {
          let nombreReal = prompt("¡Bienvenido! Ingresa tu nombre y apellido real:");
          if (nombreReal && nombreReal !== nombrePorDefecto) {
            await supabaseClient.from('usuarios').update({ nombre: nombreReal }).eq('id', currentUser.id);
            currentUser.nombre = nombreReal;
          }
        }
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    ...
    if(btnNext) btnNext.onclick = () => {
      activeWeek = window.nextWeek(activeWeek);
      renderReuniones();
    };
  }, 1000);
});
  ...
  S.reuniones.forEach(r => {
    ...
    roles.forEach(([uid, titulo]) => {
      if (uid === currentUser.id) {
        items.push({ sortKey: r.id, color: 'var(--accent-primary)', titulo, sub: `Reunión ${r.id}` });
      }
    });
  });
