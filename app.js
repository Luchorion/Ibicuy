...
async function init() {
  ...
  // Suscripción a cambios en vivo (Realtime)
    ...
    .on(
      ...
      (payload) => {
        if (payload.new && payload.new.data) {
          S = { ...S, ...payload.new.data };
          saveLocal();
          if (currentUser) { // Si el usuario está logueado, re-renderizamos la app
            if (document.getElementById('current-week-label')) {
              document.getElementById('current-week-label').textContent = activeWeek;
            }
            renderInicio();
...
async function loadData(authUser) {
  ...
  if (!currentUser) {
    ...
    if (S.usuarios.length === 0) {
      currentUser = {
        id: authUser.id, email: authUser.email, nombre: nombrePerfil,
      ...
      currentUser = {
        id: authUser.id, email: authUser.email, nombre: nombrePerfil,
        rol: 'publicador', rol_especifico: '', etiquetas: [],
        permisos: [],
        ausencias: []
      };
    }
